...
$return .= <<<EOF
Bài viết này chỉ những thành viên có số điểm trên {$creditsrequire}  mới xem được, điểm hiện tại của bạn là {$_G['member']['credits']}</div>
EOF;
?><?php return $return;?><?php }

function tpl_hide_credits($creditsrequire, $message) {?><?php
$return = <<<EOF
<div class="locked">Bạn phải có số điểm trên {$creditsrequire}  mới xem được</div>
{$message}<br /><br />
...
$return = <<<EOF
<div class="blockcode"><div id="{$randomid}"><ol><li>{$code}</ol></div><em onclick="copycode($('{$randomid}'));">Sao chép mã</em></div>
EOF;
?><?php return $return;?><?php }
...
global $_G;?><?php
$return = <<<EOF
<div class="showhide"><h4>Nội dung bài viết ẩn</h4>\\1</div>
...
$return .= <<<EOF
Nếu bạn muốn xem nội dung ẩn hãy<a href="forum.php?mod=post&amp;action=reply&amp;fid={$_G['fid']}&amp;tid={$_G['tid']}" onclick="showWindow('reply', this.href)"> trả lời </a></div>
EOF;
?><?php return $return;?><?php }
...
<div class="tip_c">
<p class="y">{$attach['dateline']} Tải lên</p>
<p>Click to download</p>

EOF;
 if(!$attach['attachimg'] && $_G['getattachcredits']) { 
$return .= <<<EOF
Tải điểm: {$_G['getattachcredits']}<br />
EOF;
 } 
$return .= <<<EOF
...
EOF;
 if($attach['readperm']) { 
$return .= <<<EOF
, Quyền đọc: <strong>{$attach['readperm']}</strong>
EOF;
 } 
$return .= <<<EOF
, Lượt tải về: {$attach['downloads']}
EOF;
 if(!$attach['attachimg'] && $_G['getattachcredits']) { 
$return .= <<<EOF
, Tải điểm: {$_G['getattachcredits']}
EOF;
 } 
$return .= <<<EOF
</p>
...
Giá: <strong>{$attach['price']} {$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['unit']}{$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['title']}</strong> &nbsp;[<a href="forum.php?mod=misc&amp;action=viewattachpayments&amp;aid={$attach['aid']}" onclick="showWindow('attachpay', this.href)" target="_blank">Danh sách mua</a>]

EOF;
 if(!$attach['payed']) { 
$return .= <<<EOF

&nbsp;[<a href="forum.php?mod=misc&amp;action=attachpay&amp;aid={$attach['aid']}&amp;tid={$attach['tid']}" onclick="showWindow('attachpay', this.href)">Mua</a>]
...
<em class="xg1">({$attach['attachsize']}, Lượt tải về: {$attach['downloads']})</em>

EOF;
 if($firstpost && $_G['fid'] && $_G['forum']['picstyle'] && ($_G['forum']['ismoderator'] || $_G['uid'] == $attach['uid'])) { 
$return .= <<<EOF

<a href="forum.php?mod=ajax&amp;action=setthreadcover&amp;aid={$attach['aid']}&amp;fid={$_G['fid']}" onclick="showWindow('setcover{$attach['aid']}', this.href)">Đặt bìa</a>
...
&nbsp;[<a href="forum.php?mod=misc&amp;action=attachpay&amp;aid={$attach['aid']}&amp;tid={$attach['tid']}" onclick="showWindow('attachpay', this.href)" target="_blank">Mua</a>]
...
<div class="tip_c xs0">
<div class="y">{$attach['dateline']} Tải lên</div>
<a href="forum.php?mod=attachment{$is_archive}&amp;aid={$aidencode}&amp;nothumb=yes" title="{$attach['filename']} Lượt tải về:{$attach['downloads']}" target="_blank"><strong>Tải đính kèm</strong> <span class="xs0">({$attach['attachsize']})</span></a>
...
<br /><a href="forum.php?mod=ajax&amp;action=setthreadcover&amp;aid={$attach['aid']}&amp;fid={$_G['fid']}" onclick="showWindow('setcover{$attach['aid']}', this.href)">Đặt bìa</a>
...
<em class="xg1">({$attach['attachsize']}, Lượt tải về: {$attach['downloads']}
EOF;
 if($attach['price']) { 
$return .= <<<EOF
, Giá: {$attach['price']} {$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['unit']}{$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['title']}
EOF;
 } 
$return .= <<<EOF
)</em>
...
$return .= <<<EOF
<br />Quyền đọc: {$attach['readperm']}
EOF;
 } } if($attach['price']) { 
$return .= <<<EOF

<br />Giá: {$attach['price']} {$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['unit']}{$_G['setting']['extcredits'][$_G['setting']['creditstransextra']['1']]['title']}	&nbsp;<a href="forum.php?mod=misc&amp;action=viewattachpayments&amp;aid={$attach['aid']}" onclick="showWindow('attachpay', this.href)" target="_blank">[Danh sách mua]</a>
...
$return .= <<<EOF
<br />Tải điểm: {$_G['getattachcredits']}
EOF;
 } 
$return .= <<<EOF
