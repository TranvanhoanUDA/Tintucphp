...
function profile_setting($fieldid, $space=array(), $showstatus=false, $ignoreunchangable = false, $ignoreshowerror = false) {
	...
	if($fieldid == 'birthday') {
		if($field['unchangeable'] && !empty($space[$fieldid])) {
			return '<span>'.$space['birthday'].'-'.$space['birthmonth'].'-'.$space['birthyear'].'</span>';
		}
		$birthyeayhtml = '';
		...
		$html = '<select name="birthday" id="birthday" class="ps" onchange="showbirthday();" tabindex="1">'
				...
				.'</select>'
				.'&nbsp;&nbsp;'
				.'<select name="birthyear" id="birthyear" class="ps" tabindex="1">'
...
function profile_show($fieldid, $space=array()) {
	...
	if($fieldid=='gender') {
		...
		if($space['birthmonth'] && $space['birthday']) {
			$return = lang('space', 'month').' '.$space['birthmonth'].' '.lang('space', 'day').' '.$space['birthday'].' ';
		}
		$return .= $space['birthyear'] ? lang('space', 'year'). ' '.$space['birthyear'].' ' : '';
		return $return;
	} elseif($fieldid=='birthcity') {
