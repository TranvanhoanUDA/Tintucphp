...
function showsetting($setname, $varname, $value, $type = 'radio', $disabled = '', $hidden = 0, $comment = '', $extra = '', $setid = '', $nofaq = false) {
	...
	if(!isset($_G['showsetting_multi'])) {
		if(!$nofaq) {
			showtablerow('onmouseover="setfaq(this, \'faq'.$setid.'\')"', 'colspan="2" class="td27" s="1"', $name);
		} else {
...
function rewritedata($alldata = 1) {
	...
	if(!$alldata) {
		...
		$data['rulesearch']['portal_article'] = 'article-{id}-{article}/{page}';
		$data['rulereplace']['portal_article'] = 'portal.php?mod=view&aid={id}&page={page}';
		$data['rulevars']['portal_article']['{id}'] = '([0-9]+)';
		$data['rulevars']['portal_article']['{article}'] = '(.*)';        
		$data['rulevars']['portal_article']['{page}'] = '([0-9]+)';

		$data['rulesearch']['forum_forumdisplay'] = 'forum-{fid}-{forum}/{page}';
		...
		$data['rulevars']['forum_forumdisplay']['{forum}'] = '(.*)';        

		$data['rulesearch']['forum_viewthread'] = 'thread-{tid}-{thread}/{page}';
		$data['rulereplace']['forum_viewthread'] = 'forum.php?mod=viewthread&tid={tid}&extra=page\%3D{prevpage}&page={page}?{thread}';
		$data['rulevars']['forum_viewthread']['{tid}'] = '([0-9]+)';
		$data['rulevars']['forum_viewthread']['{page}'] = '([0-9]+)';
		$data['rulevars']['forum_viewthread']['{prevpage}'] = '([0-9]+)';
		$data['rulevars']['forum_viewthread']['{thread}'] = '(.*)';        

		$data['rulesearch']['group_group'] = 'group-{fid}-{group}/{page}';
		$data['rulereplace']['group_group'] = 'forum.php?mod=group&fid={fid}&page={page}?{group}';
		$data['rulevars']['group_group']['{fid}'] = '([0-9]+)';
		$data['rulevars']['group_group']['{page}'] = '([0-9]+)';
		$data['rulevars']['group_group']['{group}'] = '(.*)';        

		$data['rulesearch']['home_space'] = '{user}-{value}';
		...
		$data['rulesearch']['home_blog'] = 'blog-{uid}-{blogid}-{blog}/';
		$data['rulereplace']['home_blog'] = 'home.php?mod=space&uid={uid}&do=blog&id={blogid}?{blog}';
		$data['rulevars']['home_blog']['{uid}'] = '([0-9]+)';
		$data['rulevars']['home_blog']['{blogid}'] = '([0-9]+)';
		$data['rulevars']['home_blog']['{blog}'] = '(.*)';        
		...
		$data['rulesearch']['plugin'] = 'plugin-{pluginid}-{module}.html';
