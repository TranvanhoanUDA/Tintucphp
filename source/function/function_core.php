 ...
 *		Mod SEO Discuz by Kyehani - Discuz.vn
...
function rewriteoutput($type, $returntype, $host) {
	...
	if($type == 'forum_forumdisplay') {
		...
		$r = array(
			...
			'{page}' => $page ? $page : '',
			'{forum}' => locdau(DB::result_first("SELECT name FROM ".DB::table('forum_forum')." WHERE fid='$fid'")),
		...
		$r = array(
			...
			'{thread}' => locdau(DB::result_first("SELECT subject FROM ".DB::table('forum_thread')." WHERE tid='$tid'")),
		...
		$r = array(
			'{uid}' => $uid,
			'{blogid}' => $blogid,
			'{blog}' => locdau(DB::result_first("SELECT subject FROM ".DB::table('home_blog')." WHERE blogid='$blogid'")),
		...
		$r = array(
			'{fid}' => $fid,
			'{page}' => $page ? $page : '',
			'{group}' => locdau(DB::result_first("SELECT name FROM ".DB::table('forum_forum')." WHERE fid='$fid'")),
		...
		$r = array(
			'{id}' => $id,
			'{page}' => $page ? $page : '',
			'{article}' => locdau(DB::result_first("SELECT title FROM ".DB::table('portal_article_title')." WHERE aid='$id'")),
...
function output_replace($content) {
	...
	if($_G['setting']['kyehani_seopack'] == 1){
	function rewrite_tag($pre, $next){
		$next = rawurlencode($next);
		return 'href="'.$pre.$next.'/"';
	}
	function rewrite_cat($pre, $catid, $namecat){
		$namecat = locdau(DB::result_first("SELECT name FROM ".DB::table('forum_forum')." WHERE fid='$catid'"));
		return 'href="'.$pre.$catid.$namecat.'/"';
	}
	function rewrite_uid2name($var){//Mod Sync uid with username
		$u = DB::result_first("SELECT username FROM ".DB::table('common_member')." WHERE uid='$var'");
		return 'href="member-'.$u.'"';
	}	
	$fs_search[] = "/href\=\"misc\.php\?mod\=tag&amp;name\=(.*?)\"/ie";
	$fs_replace[] = "rewrite_tag('tag-', '\\1')";

	$fs_search[] = "/href\=\"forum\.php\?gid\=([0-9]+?)\"/ie";
	$fs_replace[] = "rewrite_cat('category-', '\\1-', '\\2')";

	$fs_search[] = "/href\=\"uid-([0-9]+?)\"/ie";
	$fs_replace[] = "rewrite_uid2name('\\1')";

	$fs_search[] = "/href\=\"(forum|home|group)\.php\"/";
	$fs_replace[] = 'href="\1"';
	
	$fs_search[] = "/href\=\"(home|forum|group)\.php\?mod\=([a-z]+?)\"/";
	$fs_replace[] = 'href="\1-\2"';	

	
	$content = preg_replace($fs_search,$fs_replace,$content);}
	return $content;
}
