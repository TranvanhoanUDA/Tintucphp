<?php

/**
 *      [Discuz!] (C)2001-2099 Comsenz Inc.
 *      This is NOT a freeware, use is subject to license terms
*		Vietnamese Translation by DCV Team - Discuz.vn 2/9/2012
 *      $Id: lang_template.php 29882 2012-05-02 07:21:37Z liulanbo $
 *
 *      This file is automatically generate
 */

$lang = array (
  'getpassword' => 'Quên mật khẩu',
  'login_guest' => 'Chưa có tài khoản? <a href="member.php?mod={$_G[setting][regname]}">{$_G[setting][reglinkname]}</a>',
  'new_password' => 'Đổi mật khẩu',
  'new_password_confirm' => 'Xác nhận',
  'password_weak' => 'Độ mạnh mật khẩu',
  'strongpw_1' => 'Dùng số',
  'strongpw_2' => 'Ký tự viết thường',
  'strongpw_3' => 'Ký tự viết hoa',
  'strongpw_4' => 'Ký tự đặc biệt',
  'submit' => 'Chấp nhận',
  'close' => 'Đóng',
  'faq' => 'Trợ giúp',
  'login' => 'Đăng nhập',
  'login_clearcookies' => 'Xóa cookie',
  'login_guestmessage' => 'Bạn cần đăng nhập để tiếp tục',
  'login_member' => 'Đăng nhập',
  'login_method' => 'Đăng nhập nhanh',
  'login_permanent' => 'Ghi nhớ?',
  'profile_renew' => 'Vui lòng điền các thông tin',
  'register_from' => 'Kiến nghị',
  'account' => 'Tài khoản',
  'password' => 'Mật khẩu',
  'activation' => 'Kích hoạt',
  'agree' => 'Đồng ý',
  'disagree' => 'Không đồng ý',
  'index_activation' => 'Tài khoản của bạn cần kích hoạt',
  'invite_code' => 'Mã kích hoạt',
  'login_inactive' => 'Kích hoạt, <a href="member.php?mod={$_G[setting][regname]}">{$_G[setting][reglinkname]}</a>',
  'login_now' => 'Đã có tài khoản, đăng nhập ngay',
  'password_confirm' => 'Nhập lại',
  'register_buyinvitecode' => 'Không có mã mời, bấm vào đây để lấy',
  'register_email_tips' => 'Điền email bạn đang dùng để nhận thư kích hoạt',
  'register_message' => 'Đăng ký tại',
  'register_message1' => 'Bạn điền vào lý do đăng ký sẽ được coi như một tài liệu tham khảo quan trọng',
  'register_password_length_tips1' => 'Chiều dài tối thiểu là ',
  'register_password_length_tips2' => 'ký tự',
  'register_password_tips' => 'Điền mật khẩu',
  'register_repassword_tips' => 'Điền lại mật khẩu',
  'register_username_tips' => 'Từ 3 đến 15 ký tự',
  'register_validate_email_tips' => 'Đăng ký cần xác nhận email, hãy chắc chắn để điền vào trong hộp thư chính xác, xin vui lòng kiểm tra email của bạn sau khi đăng ký. <br /> Bạn có thể cần phải chờ một vài phút để nhận được hộp thư đến e-mail , xin vui lòng kiểm tra hộp thư rác.',
  'rulemessage' => ' Điều khoản',
);

?>