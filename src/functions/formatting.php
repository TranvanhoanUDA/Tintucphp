 ...
 * @return string
 */
function format_bytes($bytes, $to = 'M', $decimal_places = 1)
{
...
function time_ago($timestamp, $textdomain = null, $detail = 1)
{
    ...
    $retval = '';
    $periods = [__('decade', $textdomain) => 315360000, __('year', $textdomain) => 31536000, __('month', $textdomain) => 2628000, __('week', $textdomain) => 604800, __('day', $textdomain) => 86400, __('hour', $textdomain) => 3600, __('minute', $textdomain) => 60, __('second', $textdomain) => 1];
    if ($difference < 60) {
        $retval = __('just_now', $textdomain);
        return $retval;
    } else {
        foreach ($periods as $key => $value) {
            if ($difference >= $value) {
                ...
                $difference %= $value;
                $retval .= ($retval ? ' ' : '') . $time . ' ';
                $retval .= (($time > 1) ? __($key, $textdomain, ['count' => 2]) : $key);
                $detail--;
            }
            if ($detail === 0) {
                break;
        ...
        return sprintf(__('%s_ago', $textdomain), $retval);
    }
}
