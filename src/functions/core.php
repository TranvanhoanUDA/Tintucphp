...
function is_installed()
{
    ...
    if (is_null($installed)) {
        $installed = (bool) is_file(srcpath('settings/db.php'));
    }
...
/**
 * Alias of basepath() uses plugins folder as base directory
 *
 * @param  string  $path The path
 * @return string
 */
...
{
    return basepath($path, trailingslashit(BASEPATH) . trailingslashit(PLUGIN_DIR));
}
...
function html_entities($input, $doubleEncode = true)
{
    ...
    return htmlentities($input, ENT_QUOTES | ENT_HTML5, 'UTF-8', $doubleEncode);
}
 ...
 * @return
 */
function redirect_to_current_route($queryParams = true)
{
    return redirect(get_current_route_uri($queryParams));
}

function get_current_route_uri($queryParams = false)
{
    $app = app();
    ...
    if ($queryParams) {
        $query = request_build_query([], null);
        if (!empty($query)) {
            $url .= "?" . request_build_query([], null);
        }
    }
