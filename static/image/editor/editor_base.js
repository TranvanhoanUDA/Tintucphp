...
function renewContent() {
	...
	if(window.confirm('Bạn muốn khôi phục lại?')) {
		var data = loadUserdata('home');
		if(in_array((data = trim(data)), ['', 'null', 'false', null, false])) {
			parent.showDialog('Xác nhận khôi phục bản lưu！');
			return;
		}
...
function format(type, para){
	...
	if(!gIsIE){
		switch(type){
			case "Cut":
				sAlert = "Bạn cần nhấn Ctrl + X để cắt";
				break;
			case "Copy":
				sAlert = "Bạn cần nhấn Ctrl + C để sao chép";
				break;
			case "Paste":
				sAlert = "Bạn cần nhấn Ctrl + V để dán";
				break;
		}
	}
	if(sAlert != ""){
...
function doodleBox(event, id) {
	if(parent.$('uchome-ttHtmlEditor') != null) {
		...
		alert("Không thể tìm thấy dữ liệu Graffiti");
	}
}
...
function fDisplayElement(element,displayValue) {
	if(gIEVer<=5.01 && gIsIE){
		alert('Hỗ trợ IE 5 hoặc cao hơn');
		return;
	}
	fHideMenu();
...
function changeEditType(flag, ev){
	...
	try{
		...
		if(flag){
			...
			ev = ev || event;
			if(ev){
				if(window.confirm("Chuyển sang dạng văn bản sẽ bị mất một số định dạng.\nBạn có chắc chắn ?")){
