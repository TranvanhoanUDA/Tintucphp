...
function _updatesecqaa(idhash) {
	if($('secqaa_' + idhash)) {
		...
		ajaxget('misc.php?mod=secqaa&action=update&idhash=' + idhash, 'secqaa_' + idhash, null, '', '', function() {
			secST['qaa_' + idhash] = setTimeout(function() {$('secqaa_' + idhash).innerHTML = '<span class="xi2 cur1" onclick="updatesecqaa(\''+idhash+'\')">Đổi câu hỏi</span>';}, 180000);
		});
	}
}
	...
	if(isUndefined(play)) {
		if($('seccode_' + idhash)) {
			...
			ajaxget('misc.php?mod=seccode&action=update&idhash=' + idhash, 'seccode_' + idhash, null, '', '', function() {
				secST['code_' + idhash] = setTimeout(function() {$('seccode_' + idhash).innerHTML = '<span class="xi2 cur1" onclick="updateseccode(\''+idhash+'\')">Đổi mã khác</span>';}, 180000);
			});
		}
	} else {
...
function _checksec(type, idhash, showmsg, recall) {
	...
	x.get('misc.php?mod=sec' + type + '&action=check&inajax=1&&idhash=' + idhash + '&secverify=' + (BROWSER.ie && document.charset == 'utf-8' ? encodeURIComponent(secverify) : secverify), function(s){
		...
		if(s.substr(0, 7) == 'succeed') {
			...
			if(showmsg) {
				if(type == 'code') {
					showError('Sai mã. Mời nhập lại!');
				} else if(type == 'qaa') {
					showError('Sai câu hỏi an toàn!');
				}
				recall(0);
			}
		}
...
function _setCopy(text, msg){
	if(BROWSER.ie) {
		...
		if(r) {
			...
			showDialog('<div class="c"><div style="width: 200px; text-align: center;">Sao chép thất bại. Chọn "Cho phép truy cập"</div></div>', 'alert');
		}
	} else {
		var msg = '<div class="c"><div style="width: 200px; text-align: center; text-decoration:underline;">Nhấn để sao chép</div>' +
	...
	if(!obj.id) {
		...
		if(!t) {
			...
			if($(t)) {
				...
				s += '<a href="javascript:;" onclick="$(\'' + inpid + '\').value = \'0\'">Mãi mãi</a>';
...
function _zoom(obj, zimg, nocover, pn, showexif) {
	...
	if(!menu && !pn) {
		...
		menu.id = menuid;
		if(cover) {
			menu.innerHTML = '<div class="zoominner" id="' + menuid + '_zoomlayer" style="display:none"><p><span class="y"><a id="' + menuid + '_imglink" class="imglink" target="_blank" title="Mở cửa sổ mới" alt="Mở cửa sổ mới">Mở cửa sổ mới</a><a id="' + menuid + '_adjust" href="javascipt:;" class="imgadjust" title="Kích thước thực" alt="Kích thước thực">Kích thước thực</a>' +
				'<a href="javascript:;" onclick="hideMenu()" class="imgclose" title="Đóng">Đóng</a></span>Lăn chuột giữa để zoom ảnh</p>' +
				'<div class="zimg_p" id="' + menuid + '_picpage"></div><div class="hm" id="' + menuid + '_img"></div></div>';
		} else {
			menu.innerHTML = '<div class="popupmenu_popup" id="' + menuid + '_zoomlayer" style="width:auto"><span class="right y"><a href="javascript:;" onclick="hideMenu()" class="flbc" style="width:20px;margin:0 0 2px 0">Đóng</a></span>Lăn chuột giữa để zoom ảnh<div class="zimg_p" id="' + menuid + '_picpage"></div><div class="hm" id="' + menuid + '_img"></div></div>';
		}
	...
	if(typeof zoomgroup == 'object' && zoomgroup[obj.id] && typeof aimgcount == 'object' && aimgcount[zoomgroup[obj.id]]) {
		...
		if(authorlength > 1) {
			...
			if(authorcurrent !== '') {
				...
				picpage += ' <div id="zimg_prev" onmouseover="dragMenuDisabled=true;this.style.backgroundPosition=\'0 50px\'" onmouseout="dragMenuDisabled=false;this.style.backgroundPosition=\'0 -100px\';" onclick="_zoom_page(\'' + paid + '\', ' + (showexif ? 1 : 0) + ')" class="zimg_prev"><strong>Trước</strong></div> ';
				paid = authorcurrent < authorlength - 1 ? authorimgs[authorcurrent + 1] : authorimgs[0];
				picpage += ' <div id="zimg_next" onmouseover="dragMenuDisabled=true;this.style.backgroundPosition=\'100% 50px\'" onmouseout="dragMenuDisabled=false;this.style.backgroundPosition=\'100% -100px\';" onclick="_zoom_page(\'' + paid + '\', ' + (showexif ? 1 : 0) + ')" class="zimg_next"><strong>Sau</strong></div> ';
			}
			if(picpage) {
...
function _showColorBox(ctrlid, layer, k, bgcolor) {
	...
	if(!$(ctrlid + '_menu')) {
		...
		var colortexts = ['Black', 'Sienna', 'DarkOliveGreen', 'DarkGreen', 'DarkSlateBlue', 'Navy', 'Indigo', 'DarkSlateGray', 'DarkRed', 'DarkOrange', 'Olive', 'Green', 'Teal', 'Blue', 'SlateGray', 'DimGray', 'Red', 'SandyBrown', 'YellowGreen', 'SeaGreen', 'MediumTurquoise', 'RoyalBlue', 'Purple', 'Gray', 'Magenta', 'Orange', 'Yellow', 'Lime', 'Cyan', 'DeepSkyBlue', 'DarkOrchid', 'Silver', 'Pink', 'Wheat', 'LemonChiffon', 'PaleGreen', 'PaleTurquoise', 'LightBlue', 'Plum', 'White'];
		var str = '';
		for(var i = 0; i < 40; i++) {
...
function _showCreditmenu() {
	if(!$('extcreditmenu_menu')) {
		...
		menu.innerHTML = '<div class="p_opt"><img src="'+ IMGDIR + '/loading.gif" width="16" height="16" class="vm" /> Đang tải...</div>';
		$('append_parent').appendChild(menu);
