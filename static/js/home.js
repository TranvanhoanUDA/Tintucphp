...
function blogAddOption(sid, aid) {
	...
	if (newOption!=null && newOption!='') {
		...
		alert('Chuyên mục không thể để trống!');
		return false;
	}
}
		...
		theImages[i].onload = function() {
			if (this.width > size) {
				...
				if (this.parentNode.tagName.toLowerCase() != 'a') {
					...
					this.title = 'Click để xem ảnh gốc';
...
function showFlash(host, flashvar, obj, shareid) {
	...
	if(flashAddr[host]) {
		...
		var nodetxt = document.createTextNode("Thu lại");
...
function myfriend_post(uid) {
	if($('friend_'+uid)) {
		$('friend_'+uid).innerHTML = '<p>Bạn đã kết bạn với người này. Bạn có thể <a href="home.php?mod=space&do=wall&uid='+uid+'" class="xi2" target="_blank">Gửi tin nhắn</a> ，或者 <a href="home.php?mod=spacecp&ac=poke&op=send&uid='+uid+'&handlekey=propokehk_'+uid+'" id="a_poke_'+uid+'" class="xi2" onclick="showWindow(this.id, this.href, \'get\', 0, {\'ctrlid\':this.id,\'pos\':\'13\'});">Trêu chọc</a></p>';
	}
	showCreditPrompt();
}
...
function feed_more_show(feedid) {
	...
	$(opid).innerHTML = '&laquo; Mở rộng danh sách';
	$(opid).onclick = function() {
