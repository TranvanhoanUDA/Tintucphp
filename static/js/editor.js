...
function initEditor() {
	...
	for(var i = 0; i < buttons.length; i++) {
		if(buttons[i].id.indexOf(editorid + '_') != -1) {
			...
			if(buttons[i].id.substr(buttons[i].id.indexOf('_') + 1) == 'fullswitcher') {
				buttons[i].innerHTML = !editorisfull ? 'Full' : 'Trở lại';
				buttons[i].onmouseover = function(e) {setEditorTip(editorisfull ? 'Trở lại' : 'Full');};
				buttons[i].onclick = function(e) {editorfull();doane();}
			} else if(buttons[i].id.substr(buttons[i].id.indexOf('_') + 1) == 'simple') {
				buttons[i].innerHTML = !simplodemode ? 'Đơn giản' : 'Nâng cao';
	...
	if($(editorid + '_fullswitcher') && BROWSER.ie && BROWSER.ie < 7) {
		$(editorid + '_fullswitcher').onclick = function () {
			showDialog('Trình duyệt của bạn không hỗ trợ tính năng này, hãy nâng cấp phiên bản trình duyệt của bạn', 'notice', 'Thông báo');
		};
...
function savedataTime() {
	if(!autosave) {
		$(editorid + '_svdsecond').innerHTML = '<a title="Ấn vào để mở tự lưu" href="javascript:;" onclick="setAutosave()">Tự động lưu nháp</a> ';
		return;
	}
	if(!savedatac) {
		savedatac = savedataInterval;
		...
		h = h < 10 ? '0' + h : h;
		m = m < 10 ? '0' + m : m;
		setEditorTip('Bản nháp được lưu lúc ' + h + ':' + m + ' phút');
	}
	$(editorid + '_svdsecond').innerHTML = '<a title="Ấn để Tắt tự động lưu nháp" href="javascript:;" onclick="setAutosave()">Tự động lưu sau ' + savedatac + ' giây</a> ';
	savedatac -= 10;
}

function setAutosave() {
	autosave = !autosave;
	setEditorTip(autosave ? 'Lưu nháp tự động được Bật' : 'Lưu nháp tự động Tắt');
...
function editorfull(op) {
	...
	$(editorid + '_fullswitcher').innerHTML = editorisfull ? 'Trở lại' : 'Full';
	initcstbar();
}
...
function editorsimple() {
	if($(editorid + '_body').className == 'edt') {
		v = 'none';
		$(editorid + '_simple').innerHTML = 'Nâng cao';
		...
		v = '';
		$(editorid + '_simple').innerHTML = 'Cơ bản';
...
function checklength(theform) {
	...
	showDialog('Chiều dài: ' + mb_strlen(message) + ' ký tự. ' + (postmaxchars != 0 ? 'Hệ thống giới hạn từ:  ' + postminchars + ' đến ' + postmaxchars + ' ký tự' : ''), 'notice', 'Đếm ký tự');
}
...
function discuzcode(cmd, arg) {
	...
	if(in_array(cmd, ['sml', 'url', 'quote', 'code', 'free', 'hide', 'aud', 'vid', 'fls', 'attach', 'image', 'pasteword']) || typeof EXTRAFUNC['showEditorMenu'][cmd] != 'undefined' || cmd == 'tbl' || in_array(cmd, ['fontname', 'fontsize', 'forecolor', 'backcolor']) && !arg) {
		...
		if(txt = getSel()) {
			...
			while(listvalue = prompt('Nhập 1 mục danh sách.\r\n bỏ trống hoặc nhấn Cancel để hoàn thành danh sách này.', '')) {
				if(BROWSER.opera > 8) {
					listvalue = '\n' + '[*]' + listvalue;
		...
		saveData();
		setEditorTip('Dữ liệu đã được lưu');
	} else if(cmd == 'chck') {
		checklength(editorform);
	} else if(cmd == 'tpr') {
		if(confirm('Bạn có chắc chắn muốn xóa tất cả nội dung đang soạn thảo?')) {
			clearContent();
		}
	} else if(cmd == 'downremoteimg') {
		showDialog('<div id="remotedowninfo"><p class="mbn">Tải đính kèm từ xa, xin đợi......</p><p><img src="' + STATICURL + 'image/common/uploading.gif" alt="" /></p></div>', 'notice', '', null, 1);
...
function showEditorMenu(tag, params) {
	...
	if(menu) {
		...
		switch(tag) {
			case 'url':
				str = 'Nhập địa chỉ liên kết URL:<br /><input type="text" id="' + ctrlid + '_param_1" style="width: 98%" value="" class="px" />'+
					(selection ? '' : '<br />Nhập văn bản hiển thị:<br /><input type="text" id="' + ctrlid + '_param_2" style="width: 98%" value="" class="px" />');
				break;
			case 'forecolor':
			...
			case 'free':
				...
				var lang = {'quote' : 'Nhập trích dẫn', 'code' : 'Nhập đoạn mã', 'hide' : 'Nhập nội dung ẩn', 'free' : 'Nhập nội dung miễn phí nếu bạn bán chủ đề thì nội dung này được hiển thị'};
				str += lang[tag] + ':<br /><textarea id="' + ctrlid + '_param_1" style="width: 98%" cols="50" rows="5" class="txtarea"></textarea>' +
					(tag == 'hide' ? '<br /><label><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_1" class="pc" checked="checked" />Trả lời mới được xem bài</label><br /><label><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_2" class="pc" />Có đủ số điểm mới được xem bài</label> <input type="text" size="3" id="' + ctrlid + '_param_2" class="px pxs" /> Chỉ hiển thị<br /><br /><label>Có hiệu lực (ngày):</label> <input type="text" size="3" id="' + ctrlid + '_param_3" class="px pxs" /> <br />Sau bao nhiêu ngày nội dụng tự động hiển thị' : '');
				break;
			case 'tbl':
				str = '<p class="pbn">Hàng:: <input type="text" id="' + ctrlid + '_param_1" size="2" value="2" class="px" /> &nbsp; Cột: <input type="text" id="' + ctrlid + '_param_2" size="2" value="2" class="px" /></p><p class="pbn">Chiều rộng: <input type="text" id="' + ctrlid + '_param_3" size="2" value="" class="px" /> &nbsp; Chiều cao: <input type="text" id="' + ctrlid + '_param_4" size="2" class="px" onclick="showColorBox(this.id, 2)" /></p><p class="xg2 pbn" style="cursor:pointer" onclick="showDialog($(\'tbltips_msg\').innerHTML, \'notice\', \'Mẹo\', null, 0)"><img id="tbltips" title="Mẹo hướng dẫn" class="vm" src="' + IMGDIR + '/info_small.gif"> Mẹo viết nhanh</p>';
				str += '<div id="tbltips_msg" style="display: none">“[tr=Màu]” Viền<br />“[td=Chiều rộng]” Xác định chiều rộng cột<br />“[td=Cột,Hàng,Rộng]” Rộng<br /><br />Ví dụ viết nhanh<div class=\'xs0\' style=\'margin:0 5px\'>[table]<br />Name:|Discuz!<br />Version:|X1<br />[/table]</div>Với"|"Tách mỗi cột, bảng nếu"|"Với"\\|"Thay vào đó, sử dụng dòng"\\n"Thay vào đó.</div>';
				break;
			case 'aud':
				str = '<p class="pbn">Nhập địa chỉ url file nhạc:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p><p class="xg2 pbn">Link có đuôi wma mp3 ra rm <br />Ví dụ: http://server/file_nhac.mp3</p>';
				break;
			case 'vid':
				str = '<p class="pbn">Nhập địa chỉ URL video:</p><p class="pbn"><input type="text" value="" id="' + ctrlid + '_param_1" style="width: 220px;" class="px" /></p><p class="pbn">Rộng: <input id="' + ctrlid + '_param_2" size="5" value="500" class="px" /> &nbsp; Cao: <input id="' + ctrlid + '_param_3" size="5" value="375" class="px" /></p><p class="xg2 pbn">Hỗ trợ các trang chia sẻ video như Youtube<br />Hỗ trợ định dạng wmv avi rmvb mov swf flv<br />Ví dụ: http://server/movie.wmv</p>';
				break;
			case 'fls':
				str = '<p class="pbn">Nhập địa chỉ file Flash::</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p><p class="pbn">Rộng: <input id="' + ctrlid + '_param_2" size="5" value="" class="px" /> &nbsp; Cao: <input id="' + ctrlid + '_param_3" size="5" value="" class="px" /></p><p class="xg2 pbn">Hỗ trợ swf flv Flash<br />Ví dụ: http://server/flash.swf</p>';
				break;
			case 'pasteword':
			...
			default:
				...
				if(!str) {
					...
					for(var i = 1; i <= params; i++) {
						if(i != params || !haveSel) {
							str += (promptlang[i - 1] ? promptlang[i - 1] : 'Vui lòng nhập ' + i + ' thông số BBCODE:') + '<br /><input type="text" id="' + ctrlid + '_param_' + i + '" style="width: 98%" value="" class="px" />' + (i < params ? '<br />' : '');
						}
					}
				}
				break;
		...
		if(menupos == '00') {
			...
			s = '<table width="100%" cellpadding="0" cellspacing="0" class="fwin"><tr><td class="t_l"></td><td class="t_c"></td><td class="t_r"></td></tr><tr><td class="m_l">&nbsp;&nbsp;</td><td class="m_c">'
				...
				+ '<p class="o pns"><button type="submit" id="' + ctrlid + '_submit" class="pn pnc"><strong>OK</strong></button></p>'
				+ '</td><td class="m_r"></td></tr><tr><td class="b_l"></td><td class="b_c"></td><td class="b_r"></td></tr></table>';
		} else {
			s = '<div class="p_opt cl"><span class="y" style="margin:-10px -10px 0 0"><a onclick="hideMenu();return false;" class="flbc" href="javascript:;">Đóng</a></span><div>' + str + '</div><div class="pns mtn"><button type="submit" id="' + ctrlid + '_submit" class="pn pnc"><strong>OK</strong></button></div></div>';
		}
		menu.innerHTML = s;
