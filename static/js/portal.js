...
function block_delete_item(bid, itemid, itemtype, itemfrom, from) {
	var msg = itemtype==1 ? 'Bạn chắc muốn xóa dữ liệu?' : 'Bạn chắc muốn chặn dữ liệu?';
	if(confirm(msg)) {
...
function recommenditem_check() {
	...
	if(sel && sel.value) {
		...
		alert("Xin vui lòng chọn một mô-đun!");
		return false;
	}
}
...
function recommenditem_byblock(bid, id, idtype) {
	...
	if(editarea) {
		...
		if(!$('recommendback')) {
			...
			back.innerHTML = '<em id="recommendback" onclick="recommenditem_back()" class="cur1">&nbsp;&nbsp;&laquo;Quay lại</em>';
...
function toggleSettingShow() {
	if(!$('tbody_setting').style.display) {
		...
		$('a_setting_show').innerHTML = 'Mở rộng';
	} else {
...
function checkblockname(form) {
	if(!(trim(form.name.value) > '')) {
		showDialog('Mô-đun không thể để trống', 'error', null, function(){form.name.focus();});
		return false;
	}
	if(form.summary && form.summary.value) {
		...
		if(tag) {
			showBlockSummary();
			form.summary.focus();
			showDialog('Lỗi nội dung, mã HTML '+tag+' không phù hợp', 'error', null, function(){form.summary.select();});
			return false;
		}
	}
		...
		for(var i = 0; i < tags.length; i++) {
			...
			if((!preTag && endTag) || (preTag && !endTag) || preTag.length != endTag.length) {
				if(returnValue) {
					...
					showDialog('Mã HTML '+tag+' không phù hợp', 'error', null, fn, true, fn);
					return false;
				}
			}
		}
...
function blockconver(ele,bid) {
	if(ele && bid) {
		if(confirm('Bạn muốn chuyển từ '+ele.options[0].innerHTML+' sang '+ele.options[ele.selectedIndex].innerHTML)) {
...
function check_itemdata_lentgh(form) {
	if(form.title && (!strLenCalc(form.title, "titlechk", form.title.getAttribute('_maxlength')) || !form.title.value)) {
		form.title.focus();
		showDialog('Tiêu đề không thể để trống', 'error', null, function(){form.title.select();});
		return false;
	}
	if(form.summary && !strLenCalc(form.summary, "summarychk", form.summary.getAttribute('_maxlength'))) {
		form.summary.focus();
		showDialog('Bạn chưa điền nội dung', 'error', null, function(){form.summary.select();});
		return false;
	}
	return true;
}