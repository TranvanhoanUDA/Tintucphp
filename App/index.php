...
// danh sách trắng các tuyến đường
...
if (strpos($app->request->url(), '/admin') === 0 ) {   
    // kiểm tra xem url hiện tại có bắt đầu bằng / admin hay không
    // nếu vậy, hãy gọi phần mềm trung gian của chúng tôi
    $app->route->callFirst(function ($app) {
        $app->load->action('Admin/Access', 'index');
    });
 
// chia sẻ bố cục quản trị
...
// chia sẻ bố cục Blog
    ...
    // chia sẻ | tải cài đặt cho từng yêu cầu
